...
const etags = new Map<string, { etag: string; value: unknown }>();
...
export class TallyApi {
  ...
  async get<T>(path: string, query: Record<string, QueryValue> = {}): Promise<{ data: T; meta?: Record<string, unknown> }> {
    const url = this.url(path, query), cacheKey = `${this.fingerprint}:${url}`, cached = etags.get(cacheKey);
    const response = await fetch(url, { headers: this.headers(cached?.etag) });
    if (response.status === 304 && cached) return cached.value as { data: T; meta?: Record<string, unknown> };
    const value = await this.read<T>(response), etag = response.headers.get("etag");
    if (etag) etags.set(cacheKey, { etag, value });
    return value;
  }
